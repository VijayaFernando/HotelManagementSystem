Public Class CheckInForm
    ...
    Private Sub DisplayRooms()
        ...
        If RoomSQL.Das.Tables(0).Rows(0).Item("Count") = 0 Then
            MsgBox("Reservations Not Available!")

        ElseIf SQL.Das.Tables(0).Rows(0).Item("is_check_in") = 1 Then
            MsgBox("Guest Has Already Checked In!", MsgBoxStyle.Critical)
    ...
    Private Sub CheckInButton_Click(sender As Object, e As EventArgs) Handles CheckInButton.Click
        ...
        Else
            SQL.Run("UPDATE reservations SET is_check_in = REPLACE(is_check_in, 0,1)")
            SQL.Con.Close()
            MsgBox("Checked In Success!", MsgBoxStyle.Information)
            SQL.Con.Close()
            ClearTextBox(Me)
            Me.Close()
        End If
    ...
    Private Sub CheckInForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        SQL.Run("
            ...
            WHERE   reservations.is_check_in = 1 AND
                    reservations.is_check_out = 0 AND
